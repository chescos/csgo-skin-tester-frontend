<template>
  <div :key="'item' + categorySlug + itemSlug">
    ...
    <div class="flex flex-wrap -mx-4">
      <div v-for="skin in orderedSkins" :key="skin.id" class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 p-4 flex-none">
        <div v-if="skin.id === configurator" style="min-height: 290px;"
          ...
          <svg v-on:click="closeConfigurator()"
               class="fill-current text-gray-500 hover:text-white absolute p-1 w-8 h-8 cursor-pointer"
               viewBox="0 0 20 20" style="top: 4px; right: 4px;" xmlns="http://www.w3.org/2000/svg">
            <path
          ...
          <form class="py-3 px-8">
            <div class="mb-8">
              ...
              <input v-model="config.seed"
                     class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                     id="seed" type="text" placeholder="Seed (1-1000)">
            </div>
            <div class="mb-6">
              <label
                for="toggle-stattrak"
              ...
              >
                ...
                <div class="ml-6 font-medium"
                     :class="{ 'text-white': !config.stattrak, 'text-stattrak': config.stattrak, }">{{ config.stattrak ?
                  'StatTrak™' : 'Normal' }}
                </div>
              </label>
            </div>
            <div>
              ...
              <vue-slider id="wear" v-model="config.wear" :min="0.01" :max="0.99" :interval="0.01"
                          tooltipPlacement="bottom" tooltip="always"></vue-slider>
            </div>
            <button v-on:click="equipSkin(skin.id)"
                    class="absolute bottom-0 left-0 right-0 w-full rounded-b bg-blue-700 hover:bg-blue-600 text-white font-bold py-2 px-4 focus:outline-none focus:shadow-outline"
                    type="button">
              Submit
            </button>
          </form>
        ...
        <div v-else class="border-4 border-gray-800 rounded shadow-md cursor-pointer relative flex flex-wrap"
          ...
          <svg v-on:click="configureSkin(skin.id)"
               class="fill-current text-gray-500 hover:text-white absolute top-0 right-0 p-1 w-8 h-8 cursor-pointer"
...
export default {
  ...
  computed: {
    ...
    orderedSkins() {
      const { skins } = this.item;

      return skins.sort((a, b) => {
        if (a.paintkit.name < b.paintkit.name) {
          return -1;
        }
        if (a.paintkit.name > b.paintkit.name) {
          return 1;
